...
class DocumentApiIntegrationTest {
	...
	void createResume_Success() throws Exception {
		...
		// when & then
		mockMvc.perform(
				multipart("/api/v1/resumes").file(file).file(requestPart).contentType(MediaType.MULTIPART_FORM_DATA))
			.andDo(print())
	...
	void createPortfolio_Success() throws Exception {
		...
		// when & then
		mockMvc.perform(
				multipart("/api/v1/portfolios").file(file).file(requestPart).contentType(MediaType.MULTIPART_FORM_DATA))
			.andDo(print())
			.andExpect(status().isCreated())
			.andExpect(jsonPath("$.success").value(true));
	}
	...
	void createEducation_Success() throws Exception {
		...
		// when & then
		mockMvc.perform(
				multipart("/api/v1/educations").file(file).file(requestPart).contentType(MediaType.MULTIPART_FORM_DATA))
			.andDo(print())
			.andExpect(status().isCreated())
			.andExpect(jsonPath("$.success").value(true));
	}
